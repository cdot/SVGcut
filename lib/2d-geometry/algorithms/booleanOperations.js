...
import { addToIntPoints, sortIntersections, filterDuplicatedIntersections, initializeInclusionFlags, calculateInclusionFlags, setOverlappingFlags, intPointsPoolCount, splitByIntersections, } from '../data_structures/smart_intersections.js';
const { Inclusion, Overlap } = Constants;
const { START_VERTEX, END_VERTEX } = Constants;
export var BooleanOp;
(function (BooleanOp) {
    BooleanOp[BooleanOp["UNION"] = 1] = "UNION";
    BooleanOp[BooleanOp["INTERSECT"] = 2] = "INTERSECT";
    BooleanOp[BooleanOp["SUBTRACT"] = 3] = "SUBTRACT";
})(BooleanOp || (BooleanOp = {}));
/**
...
function fixBoundaryConflicts(poly1, poly2, int_points1, int_points1_sorted, int_points2, intersections) {
    ...
    for (let i = 0; i < num_int_points; i++) {
        ...
        if ((edge_from1.bv === Inclusion.INSIDE && edge_to1.bv === Inclusion.OUTSIDE) ||
            (edge_from1.bv === Inclusion.OUTSIDE && edge_to1.bv === Inclusion.INSIDE)) {
...
export function removeNotRelevantChains(polygon, op, int_points, is_res_polygon) {
    ...
    for (let i = 0; i < int_points.length; i++) {
        ...
        if ((edge_from.bv === Inclusion.INSIDE && edge_to.bv === Inclusion.INSIDE && op === BooleanOp.UNION) ||
...
function removeNotRelevantNotIntersectedFaces(polygon, notIntersectedFaces, op, is_res_polygon) {
    for (let face of notIntersectedFaces) {
        ...
        if ((op === BooleanOp.UNION && rel === Inclusion.INSIDE) ||
