...
export function parsePath(description, options = {}) {
    const allParts = [[]];
    let parts = allParts[allParts.length - 1];
    let current = new Point();
    let anchor = current;
    let lastBezier = undefined;
    let lastQuadratic = undefined;
    let offset = 0;
    ...
    while (offset < length) {
        ...
        switch (code) {
            ...
            case CHAR.a: {
                ...
                parts.push(new Arc(center, r, startAngle, endAngle, sweep ? !large : large));
                current = end;
                break;
            }
            /* Quadratic */
