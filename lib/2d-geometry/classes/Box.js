...
export class Box extends Shape {
     ...
     */
    intersect(otherBox) {
        return !(this.xmax < otherBox.xmin || this.xmin > otherBox.xmax || this.ymax < otherBox.ymin || this.ymin > otherBox.ymax);
    }
    /**
     * Returns new box merged with other box
     */
    merge(otherBox) {
        return new Box(Math.min(this.xmin, otherBox.xmin), Math.min(this.ymin, otherBox.ymin), Math.max(this.xmax, otherBox.xmax), Math.max(this.ymax, otherBox.ymax));
    }
    /**
     ...
     */
    rotate(_angle, _center = Point.EMPTY) {
    ...
    static comparableMax(a, b) {
        return a.merge(b);
    }
    static comparableLessThan(a, b) {
        return a.lessThan(b);
    }
}
Box.EMPTY = Object.freeze(new Box(0, 0, 0, 0));
/** Abstract box that has no dimensions (xmin is Infinity, xmax is -Infinity, etc) */
Box.VOID = Object.freeze(new Box(Infinity, Infinity, -Infinity, -Infinity));
/**
 * Shortcut to create new box
 */
export const box = (xmin, ymin, xmax, ymax) => new Box(xmin, ymin, xmax, ymax);
