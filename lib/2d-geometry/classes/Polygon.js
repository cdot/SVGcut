...
import * as Relations from '../algorithms/relation.js';
import { addToIntPoints, calculateInclusionFlags, filterDuplicatedIntersections, getSortedArray, getSortedArrayOnLine, initializeInclusionFlags, insertBetweenIntPoints, splitByIntersections, } from '../data_structures/smart_intersections.js';
import { Multiline } from './Multiline.js';
import { intersectEdge2Line } from '../algorithms/intersection.js';
...
import { Shape, ShapeTag, MAX_EDGE_SHAPE_TAG } from './Shape.js';

const isPointLike = (n) => Array.isArray(n) && n.length === 2 && typeof n[0] === 'number' && typeof n[1] === 'number';
const isPoints = (e) => Array.isArray(e) && (e.length === 0 || e[0] instanceof Point || isPointLike(e[0]));

/**
 * Class representing a polygon.
 * Polygon in FlattenJS is a multipolygon comprised from a set of [faces]{@link Face}.
 * Face, in turn, is a closed loop of [edges]{@link Edge}, where edge may be segment
 * or circular arc
 */
export class Polygon extends Shape {
    constructor(arg) {
        ...
        if (Array.isArray(arg)) {
            if (arg.length === 0) {
                ...
                this.faces.add(new Face(this, arg));
            }
            else if (arg[0].tag <= MAX_EDGE_SHAPE_TAG) {
                ...
                for (let loop of arg) {
                    ...
                    if (Array.isArray(loop) && isPoints(loop[0])) {
                        for (let subloop of loop) {
                            this.faces.add(new Face(this, subloop));
                        }
                    }
                    else {
        ...
        else if (arg instanceof Box) {
            this.faces.add(new Face(this, arg));
        }
        else if (arg instanceof Circle) {
            this.faces.add(new Face(this, arg));
        }
     ...
     */
    get box() {
        return [...this.faces].reduce((acc, face) => acc.merge(face.box), Box.VOID);
    }
    /**
     * Returns bounding box of the polygon
     */
    ...
    cut(multiline) {
        ...
        for (const edge of multiline) {
            if (edge.setInclusion(this) !== Inclusion.INSIDE)
                continue;
    ...
    cutWithLine(line) {
        ...
        for (let int_point1_curr of intersections.int_points1_sorted) {
            if (int_point1_curr.edge_before.bv === Inclusion.INSIDE) {
                new_edge = new Edge(new Segment(int_point1_prev.pt, int_point1_curr.pt)); // (int_point1_curr.edge_before.shape);
                ...
                new_edge = new Edge(new Segment(int_point1_curr.pt, int_point1_prev.pt)); // (int_point1_curr.edge_before.shape.reverse());
    ...
    distanceTo(shape) {
        ...
        if (shape instanceof Polygon) {
            let min_dist_and_segment = [Number.POSITIVE_INFINITY, Segment.EMPTY];
            for (let edge of this.edges) {
...
Polygon.EMPTY = Object.freeze(new Polygon([]));
/**
 * Shortcut method to create new polygon
 */
export const polygon = (a) => new Polygon(a);
