...
export class Segment extends Shape {
    constructor(a, b, c, d) {
        ...
        if (argsLength === 1 && a.name === 'segment') {
            let { start, end } = a;
    ...
    pointAtLength(length) {
        ...
        return new Point((this.end.x - this.start.x) * factor + this.start.x, (this.end.y - this.start.y) * factor + this.start.y);
    }
...
Segment.EMPTY = Object.freeze(new Segment(Point.EMPTY, Point.EMPTY));
/**
 * Shortcut method to create new segment
 */
export const segment = (x1, y1, x2, y2) => new Segment(x1, y1, x2, y2);
