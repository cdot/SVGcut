...
export class Bezier extends Shape {
    ...
    get lut() {
        if (this._lut === EMPTY) {
            this._lut = curves.bezier.generateLUT(this.start.x, this.start.y, this.control1.x, this.control1.y, this.control2.x, this.control2.y, this.end.x, this.end.y);
        }
        return this._lut;
    }
    /**
    ...
    transform(matrix = new geom.Matrix()) {
        return new Bezier(this.start.transform(matrix), this.control1.transform(matrix), this.control2.transform(matrix), this.end.transform(matrix));
    }
    /**
