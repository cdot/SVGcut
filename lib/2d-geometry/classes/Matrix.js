 ...
 */
export class Matrix {
    static fromTransform(x, y, rotation, scale) {
        return new Matrix(+scale * Math.cos(rotation), +scale * Math.sin(rotation), -scale * Math.sin(rotation), +scale * Math.cos(rotation), x, y);
    }
    /**
    ...
    get inverse() {
        var _a;
        const inverse = (_a = this._inverse) !== null && _a !== void 0 ? _a : (this._inverse = this.invert());
     ...
     */
    equalTo(matrix) {
        if (!EQ(this.tx, matrix.tx))
            return false;
        if (!EQ(this.ty, matrix.ty))
            return false;
        if (!EQ(this.a, matrix.a))
            return false;
        if (!EQ(this.b, matrix.b))
            return false;
        if (!EQ(this.c, matrix.c))
            return false;
        if (!EQ(this.d, matrix.d))
            return false;
        return true;
    }
    isIdentity() {
...
Matrix.EMPTY = Object.freeze(new Matrix(0, 0, 0, 0, 0, 0));
Matrix.IDENTITY = Object.freeze(new Matrix(1, 0, 0, 1, 0, 0));
/**
 * Function to create matrix equivalent to "new" constructor
 */
export const matrix = (a, b, c, d, tx, ty) => new Matrix(a, b, c, d, tx, ty);
