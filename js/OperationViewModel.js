...
class OperationViewModel extends ViewModel {
  ...
  recombine() {
    ...
    let geom = new CutPaths();
    switch (this.combineOp()) {
    case "Intersect": geom = geom.intersection(this.operandPaths); break;
    case "Diff":      geom = geom.diff(this.operandPaths); break;
    case "Xor":       geom = geom.xor(this.operandPaths); break;
    default:          geom = geom.union(this.operandPaths); break;
    }
    ...
    if (previewGeometry.length > 0) {
      const svg = previewGeometry.toSegments();
      if (svg) {
  ...
  generateToolPaths() {
    ...
    const spaths = this.toolPaths().toSegments();
