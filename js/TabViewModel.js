...
export class TabViewModel extends ViewModel {
   ...
   * @param {ClipperLib.Paths} tabPaths input paths to the operation
   */
  ...
  recombine() {
    ...
    for (const path of this.tabPaths) {
      if (this.combinedGeometry)
        this.combinedGeometry = Clipper.clip(
          this.combinedGeometry, path, ClipperLib.ClipType.ctUnion);
      else
        this.combinedGeometry = path;
    }

    const off = this.margin.toUnits("integer");
    if (off !== 0) // bloat/shrink
      this.combinedGeometry = Clipper.offset(this.combinedGeometry, off);

    if (this.combinedGeometry.length > 0) {
      const svg = Clipper.integer2SVG(this.combinedGeometry);
      if (svg) {
