...
export class OperationViewModel extends ViewModel {
   ...
   */
  toolpathGenerator = new (GENERATORS.Engrave)();

  /**
   * Determine which of the operation fields needs to be enabled for this
   * operation.
   */
  needs = ko.observable(this.toolpathGenerator.needs);

  /**
   * The operation name. Default is Engrave as it's simplest.
   * @member {observable.<string>}
   */
  operation = ko.observable(DEFAULT.OPERATION);
   ...
   * @member {observable.<boolean>}
   */
  ramp = ko.observable(DEFAULT.RAMP);

  /**
   * Either "Conventional" or "Climb". See README.md
   * @member {observable.<string>}
   */
  direction = ko.observable(DEFAULT.DIRECTION);

  /**
   * Offset for engrave and perforate.
   * @member {observable.<string>}
   */
  offset = ko.observable(DEFAULT.OFFSET);

  /**
   * Pocketing strategy.
   * @member {observable.<string>}
   */
  strategy = ko.observable(DEFAULT.STRATEGY);

  /**
   * Paths taken by the tool to execute this operation.
   * @member {observable.<CutPaths>}
   */
  toolPaths = ko.observable(new CutPaths());

  /**
   * Maximum depth to cut to.
   * @member {observable.<number>}
   */
  cutDepth = this.limited("CUT_DEPTH");

  /**
   * Amount of material to leave uncut.
   * @member {observable.<number>}
   */
  margin = ko.observable(DEFAULT.MARGIN);

  /**
   * Spacing of perforations.
   * @member {observable.<number>}
   */
  spacing = this.limited("SPACING");
   ...
   * @member {observable.<number>}
   */
  width = ko.observable(DEFAULT.WIDTH).extend({ MIN: 0 });

  /**
   * Override Tool Defaults
   * @member {observable.<number>}
   */
  passDepth = this.limited("PASS_DEPTH", true);

  /**
   * Override Tool Defaults
   * @member {observable.<number>}
   */
  stepOver = ko.observable()
   ...
   * @member {observable.<number>}
   */
  cutRate = this.limited("CUT_RATE", true);

  /**
   * Override Tool Defaults
   * @member {observable.<number>}
   */
  rpm = ko.observable().extend({ MIN_NULL: 0 });
  ...
  constructor(unitConverter, operandPaths) {
    ...
    unitConverter.add(this.cutDepth, "cutDepth");
    ...
    unitConverter.add(this.margin, "margin2");
    this.margin.subscribe(() => {
      document.dispatchEvent(new Event("PROJECT_CHANGED"));
      this.recombine();
    });

    unitConverter.add(this.spacing, "spacing2");
    this.spacing.subscribe(() => {
      document.dispatchEvent(new Event("PROJECT_CHANGED"));
      this.recombine();
    });

    unitConverter.add(this.width, "width2");
    this.width.subscribe(() => {
      document.dispatchEvent(new Event("PROJECT_CHANGED"));
      this.recombine();
    });

    unitConverter.add(this.passDepth, "passDepth2");
    ...
    unitConverter.add(this.cutRate, "cutRate2");
